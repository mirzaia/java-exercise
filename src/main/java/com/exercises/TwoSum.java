...
public class TwoSum {
    ...
    public int[] twoSum(int[] nums, int target) {
      // updated solution
      HashMap<Integer, Integer> incomingNum = new HashMap<>();
      for (int i = 0; i < nums.length; i++) {
        incomingNum.put(nums[i], i);
      }
      // get complement then compare with existing map values
      for (int y = 0; y < nums.length; y++) {
        int complement = target - nums[y];
        if (incomingNum.containsKey(complement) && incomingNum.get(complement) != y) {
          return new int[]{y, incomingNum.get(complement)};
        }
      }
...
//
//     public int[] twoSum(int[] nums, int target) {
//       HashMap<Integer> map = new ArrayList<Integer>();
//       for (int i = 0; i < nums.length; i++) {
//          map.put(nums[i], i);
//       }
//       for (int i = 0; i < nums.length; i++) {
...
//  public int[] twoSumUser(int[] nums, int target) {
//      // map incoming array
//      HashMap<Integer, Integer> incomingNum = new HashMap<>();
//      for (int i = 0; i < nums.length; i++) {
//        incomingNum.put(nums[i], i);
...
//      for (int y = 0; y < nums.length; y++) {
//        int complement = target - nums[y];
//        if (incomingNum.containsKey(complement) && complement != nums[y]) {
//          return new int[]{y, incomingNum.get(complement)};
//        }
//      }
//    throw new IllegalArgumentException("No solution");
